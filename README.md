...
1 - 안전 길찾기 :  현재 위치를 GPS로 인식하고, CCTV의 위도,경도 값을 받아와서 목적지까지 CCTV가 있는 안전한 길로 안내해준다.<br>
2. 근처 CCTV, 지구대 표시 : 근처 CCTV위치와 지구대의 위치를 확인하고, 위급한 상황시 지구대로 연락 가능.<br>
3. 긴급 메세지 : 보호자에게 현재 사용자의 위치를 제공해주고, 위급한 상황시 메시지 전송 가능.<br>
4. 버튼 이벤트 : 알람 기능, 후레시 기능은 어플 내에서 간단한 조작으로 사용할 수 있고, 볼륨버튼을 3번 누르면 자동으로 메세지 전송 가능.<br>
